...
@Service
public class BmizhiService {

    @Resource
    private BmizhiMapper mapper;

    public ResponseData<Bmizhi> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Bmizhi>> selectAll(Wrapper<Bmizhi> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Bmizhi>> selectAll() {
        QueryWrapper<Bmizhi> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Bmizhi> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Bmizhi> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Bmizhi> wrapper = Wrappers.query();
        ...
        if (!StringUtil.isNullOrEmpty(map.get("shengao_start"))) {
            wrapper.ge("shengao", map.get("shengao_start"));
        }
        if (!StringUtil.isNullOrEmpty(map.get("shengao_end"))) {
            wrapper.le("shengao", map.get("shengao_end"));
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Bmizhi>> filter(Map<String, Object> map) {
        Class<Bmizhi> aClass = Bmizhi.class;
        com.jntoo.db.QueryWrapper<Bmizhi> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Bmizhi> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Bmizhi entityData, Map post) {
        Info.handlerNullEntity(entityData);

        mapper.insert(entityData);
        if (entityData.getId() != null) {
            Integer charuid = entityData.getId();
            DB.execute("UPDATE bmizhi SET bmizhi=(tizhong/((shengao/100)*(shengao/100))) WHERE id='" + charuid + "'");
    ...
    public ResponseData<Object> update(Bmizhi entityData, Map post) {
        mapper.updateById(entityData);

        Integer charuid = entityData.getId();
        DB.execute("UPDATE bmizhi SET bmizhi=(tizhong/((shengao/100)*(shengao/100))) WHERE id='" + post.get("id") + "'");
