...
public class YuyueService {
    ...
    public ResponseData<Yuyue> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Yuyue>> selectAll(Wrapper<Yuyue> query) {
    ...
    public QueryWrapper<Yuyue> mapToWrapper(Map<String, Object> map) {
        ...
        String where = " 1=1 ";
        // 判断URL 参数kechengid是否大于0
        if (!StringUtil.isNullOrEmpty(map.get("kechengid")) && Convert.toInt(map.get("kechengid")) > 0) {
            // 大于0 则写入条件
            wrapper.eq("kechengid", map.get("kechengid"));
        }
        ...
        if (!StringUtil.isNullOrEmpty(map.get("yuyueshijian_start"))) {
            wrapper.ge("yuyueshijian", map.get("yuyueshijian_start"));
        }
        if (!StringUtil.isNullOrEmpty(map.get("yuyueshijian_end"))) {
            wrapper.le("yuyueshijian", map.get("yuyueshijian_end"));
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Yuyue>> filter(Map<String, Object> map) {
        Class<Yuyue> aClass = Yuyue.class;
        com.jntoo.db.QueryWrapper<Yuyue> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Yuyue> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Yuyue entityData, Map post) {
    ...
    public ResponseData<Object> update(Yuyue entityData, Map post) {
