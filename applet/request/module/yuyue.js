...
const readStore = require("./kecheng");
const app = getApp();
...
var content = (actions = {
    ...
    findById(id) {
        return new Promise((resolve, reject) => {
            httpGet("/yuyue/findById", { id }).then((res) => {
                if (res.code == 0) {
                    resolve(res.data);
                } else {
                    reject(res.msg);
    ...
    filter(data) {
        return new Promise((resolve, reject) => {
            httpPost("/yuyue/filter", data).then(
                (res) => {
                    if (res.code == 0) {
                        resolve(res);
                    } else {
    ...
    query(data) {
        // 分页查询数据
        return new Promise((resolve, reject) => {
            httpPost("/yuyue/selectPages", data).then(
                (res) => {
                    if (res.code == 0) {
                        resolve(res);
                    } else {
    ...
    querytianjiaren(data) {
        // 分页查询数据
        return new Promise((resolve, reject) => {
            httpPost("/yuyue/selectPagesTianjiaren", data).then(
                (res) => {
                    if (res.code == 0) {
                        resolve(res);
                    } else {
    ...
    queryyuyueren(data) {
        // 分页查询数据
        return new Promise((resolve, reject) => {
            httpPost("/yuyue/selectPagesYuyueren", data).then(
                (res) => {
                    if (res.code == 0) {
                        resolve(res);
                    } else {
    ...
    update(data) {
        // 更新数据
        return new Promise((resolve, reject) => {
            httpPost("/yuyue/update", data).then(
                (res) => {
                    if (res.code == 0) {
                        if (content.state.status == 0) {
    ...
    insert(data) {
        // 插入数据
        return new Promise((resolve, reject) => {
            httpPost("/yuyue/insert", data).then(
                (res) => {
                    if (res.code == 0) {
                        if (content.state.status == 0) {
                            content.dispatch("selectAll");
    ...
    checkForm(data, readMap) {
        return new Promise((resolve, reject) => {
            ...
            rules.yuyueshijian = [{ required: true, message: "请填写预约时间" }];
    ...
    delete(data) {
        // 删除数据
        return new Promise((resolve, reject) => {
            httpPost("/yuyue/delete", isArray(data) ? data : [data]).then(
                (res) => {
                    if (res.code == 0) {
                        if (content.state.status == 0) {
                            content.dispatch("selectAll");
    ...
    createForm() {
        var form = {
            ...
            yuyueshijian: rule.date("Y-m-d H:i:s"),
            yuyuerenxingming: "",
            dianhua: "",
            beizhu: "",
            zhuangtai: "已预约",
            yuyueren: app.session("username"),
            kechengid: 0,
        };
        return form;
    },
