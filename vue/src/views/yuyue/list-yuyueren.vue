        ...
        <el-card class="box-card">
                ...
                <el-form @submit.prevent.stop :inline="true" size="mini">
                    ...
                    <el-form-item label="类型">
                        <el-select v-model="search.leixing"
                            ...
                            ><e-select-option type="option" module="kechengleixing" value="id" label="leixingmingcheng"></e-select-option
                        ></el-select>
                    </el-form-item>
                    <el-form-item label="预约时间">
                        <el-date-picker v-model="search.yuyueshijian_start" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" placeholder="选择日期"> </el-date-picker
                        >-<el-date-picker v-model="search.yuyueshijian_end" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" placeholder="选择日期"> </el-date-picker>
                    </el-form-item>
            ...
            <el-table border :data="list" style="width: 100%" highlight-current-row>
                ...
                <el-table-column label="类型" width="80">
                    <template slot-scope="{row}"> <e-select-view module="kechengleixing" :value="row.leixing" select="id" show="leixingmingcheng"></e-select-view> </template>
                </el-table-column>
                <el-table-column label="添加人" width="80">
                    <template slot-scope="{row}"> {{ row.tianjiaren }} </template>
                </el-table-column>
                <el-table-column label="预约时间" width="120">
                    <template slot-scope="{row}"> {{ row.yuyueshijian }} </template>
                </el-table-column>
                <el-table-column label="预约人姓名" width="130">
                    <template slot-scope="{row}"> {{ row.yuyuerenxingming }} </template>
                </el-table-column>
                <el-table-column label="电话" width="130">
                    <template slot-scope="{row}"> {{ row.dianhua }} </template>
                </el-table-column>
                ...
                <el-table-column label="预约人" width="80">
                    <template slot-scope="{row}"> {{ row.yuyueren }} </template>
                </el-table-column>
                    ...
                    <template slot-scope="{row}">
                        <el-button-group>
                            ...
                            <el-tooltip content="详情" placement="top">
                                <el-button @click="$goto({path:'/admin/yuyuedetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                            </el-tooltip>
                            <el-tooltip content="编辑" placement="top">
                                <el-button icon="el-icon-edit" @click="$goto({path:'/admin/yuyueupdt',query:{id:row.id } })" type="warning" size="mini"></el-button>
                            </el-tooltip>
    ...
    export default {
        ...
        methods: {
            ...
            loadList(page) {
                ...
                filter.session_name = "yuyueren";

                this.$store
                    .dispatch("yuyue/query", filter)
                    .then((res) => {
                        this.loading = false;
            ...
            deleteItems(items) {
                this.$confirm("确定删除数据？此操作不可恢复", "警告", {
                    ...
                    this.$store.dispatch("yuyue/delete", ids).then((res) => {
                        this.loading = false;
                        if (res.code != api.code.OK) {
